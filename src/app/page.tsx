  ...
  return (
    <main className="flex flex-col gap-8">
      ...
      {repositories.map((repository) => (
        <div
          className="border border-transparent bg-zinc-700 rounded p-8 flex flex-col gap-4 hover:border-sky-500"
          key={repository.id}
        >
          <div className="flex flex-col gap-2">
            <div>
              <small className="text-zinc-500 flex justify-between items-center">
                ...
                {repository.language ? (
                  <span className="bg-pink-600 text-pink-200 rounded p-1">
                    {repository.language}
                  </span>
                ) : null}
              </small>
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm">{repository.description}</p>
          </div>
          <div className="flex">
            {repository.topics.map((topic) => (
              <span
                className="bg-fuchsia-300 rounded p-1 mr-2 text-fuchsia-900 text-sm"
                key={topic}
              >
                {topic}
              </span>
